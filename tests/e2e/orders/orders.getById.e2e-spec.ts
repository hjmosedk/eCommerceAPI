...
describe('IntegrationsTest for orders Get By Id - The integrationTest have been split into several file to keep the file size low', () => {
  test('Should return the order with the given id', async () => {
    const order = await request(app.getHttpServer()).get('/orders/1');

    expect(order.body).toBeDefined();
    expect(order.body.id).toBe(1);
  });

  test('Should return 404 if the order does not exist', async () => {
    const order = await request(app.getHttpServer()).get('/orders/100');
