...
describe('OrderService - Get All - The test files have been spilt up in multiple files for better maintainability', () => {
      ...
      test('A list of orders and total count is returned', async () => {
        ...
        jest.spyOn(orderRepository, 'createQueryBuilder').mockReturnValue(
          createMockQueryBuilder({
            getOne: null,
            getManyAndCount: [orders, orders.length],
          }) as any,
        );
        ...
        expect(result).toEqual([orderDtos, orders.length]);
      }),
